...
InnoDB 引擎是支持行级锁的，而 MyISAM 引擎并不支持行级锁，所以以下内容都是基于 InnoDB 引擎的。
...
从上图可以看到，加锁情况与我们分析的一致，加锁范围出了普通索引c(INDEX_NAME都是idx_c)的(0,5]区间(next-key lock(LOCK_MODE为S))
...
但是实现上，InnoDB会往前扫描到第一个不满足条件的行为止，也就是id=20, 因此索引id上的(15, 20]这个next-key lock 也会被
