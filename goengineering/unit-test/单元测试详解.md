...
> **重构示例**：本目录的 `order_service.go` 对上面的 ProcessOrder 进行了重构，将其拆分为
> `ValidateStock`、`ApplyCoupon`、`CalculateTotal`、`PlaceOrder` 四个方法，并通过接口注入
> `ProductRepo`、`CouponRepo`、`OrderRepo`、`Notifier` 四个依赖。对应的测试见
> `order_service_test.go`，每个方法都可以用手写 Stub 独立测试。
...
## 5.1 接口 Mock vs gomonkey 对比

除了 gomonkey 之外，Go 社区还有另外两种常用的 Mock 方式。下面是三种方式的对比：

| 方式 | 依赖 | 优点 | 缺点 | 适用场景 |
|------|------|------|------|---------|
| **手写 Stub** | 无 | 零依赖、灵活、易读 | 接口方法多时代码量大 | 接口方法 ≤5 个，推荐首选 |
| **gomock** | mockgen 代码生成 | 自动生成、严格调用验证（次数、顺序） | 需要代码生成步骤，学习成本稍高 | 接口方法多、需要验证调用契约 |
| **gomonkey** | runtime patch | 不依赖接口，可 mock 任意函数 | 不支持 `-race`，平台限制，需 `-gcflags=all=-l` | 遗留代码、无法修改接口的场景 |

**推荐策略**：优先使用**手写 Stub + 接口注入**（见 `mock_interface.go`、`mock_interface_test.go`），
这是 Go 社区最推崇的方式，零依赖且与 `-race` 兼容。只有在无法修改被测代码接口时，才考虑 gomonkey。

gomock 的典型用法（仅作示例，未引入依赖）：

```go
// 1. 生成 mock：mockgen -source=mock_interface.go -destination=mock_user_store.go
// 2. 在测试中使用：
//    ctrl := gomock.NewController(t)
//    mockStore := NewMockUserStore(ctrl)
//    mockStore.EXPECT().GetByID("1").Return(&User{ID: "1", Name: "Alice"}, nil)
...
ok      go-notes/goengineering/unit-test    1.672s
...
# 7 TestMain：全局 Setup 与 Teardown

`TestMain(m *testing.M)` 是整个 package 测试的入口点，在所有测试函数之前执行。它提供了全局资源初始化和清理的机会。

## 7.1 生命周期

```
TestMain 开始
  ├── Setup（创建临时目录、设置环境变量、启动数据库等）
  ├── m.Run()  ← 执行所有 TestXxx 函数
  │     ├── TestA
  │     ├── TestB
  │     └── ...
  ├── Teardown（清理临时文件、关闭连接等）
  └── os.Exit(exitCode)
```
...
1. **一个 package 只能有一个 TestMain**
2. 必须调用 `m.Run()` 来执行测试，否则不会运行任何测试
3. 必须调用 `os.Exit()` 传递退出码
4. TestMain 中的 panic 不会被 `go test` 优雅处理

## 7.4 TestMain vs t.Cleanup

| 维度 | TestMain | t.Cleanup |
|------|----------|-----------|
| 作用域 | 整个 package | 单个测试/子测试 |
| 适用场景 | 昂贵的全局资源 | 单个测试的临时资源 |
| 执行时机 | 所有测试前/后各一次 | 每个测试结束时 |
| 能否多个 | 否（一个 package 只能一个） | 是（可注册多个） |

> 代码示例：`testmain_test.go`
...
Go 1.22 之前，`for range` 循环变量在所有迭代中共享同一个地址。配合 `t.Parallel()` 使用时，所有子测试闭包会捕获到最后一个迭代值：
...
Go 1.22+ 每次迭代创建新的变量，`tt := tt` 不再必要（但写了也无害）。本仓库使用 Go 1.24。

> 代码示例：`parallel_test.go`
> 陷阱演示：`trap/loop-capture/`

# 9 httptest：HTTP Handler 测试

Go 标准库 `net/http/httptest` 提供了两种测试 HTTP handler 的方式。

## 9.1 httptest.NewRecorder（单元测试风格）
...
assert(strings.Contains(rec.Body.String(), "Alice"))
```

**优点**：快速、隔离，适合纯单元测试。

## 9.2 httptest.NewServer（集成测试风格）

启动真实 HTTP 服务器（随机端口），使用标准 `http.Client`：

```go
srv := httptest.NewServer(http.HandlerFunc(handler.GetUser))
defer srv.Close()

resp, _ := http.Get(srv.URL + "?id=1")
```

**优点**：测试完整的 HTTP 请求/响应周期，包括中间件、路由、Content-Type 协商等。

## 9.3 选择建议

| 场景 | 推荐方式 |
|------|---------|
| 测试单个 handler 逻辑 | NewRecorder |
| 测试中间件链 | NewServer |
| 测试路由匹配 | NewServer |
| CI 快速反馈 | NewRecorder |

> 代码示例：`httptest_handler.go`、`httptest_handler_test.go`

# 10 接口 Mock vs gomonkey 对比

详见第 5.1 节的对比表。本目录提供了完整的手写 Stub 示例：

- `mock_interface.go`：接口定义 + `StubUserStore` + `UserService`
- `mock_interface_test.go`：表驱动测试，覆盖正常/错误注入/未找到等场景
...
通过设置函数字段来控制每个测试用例的返回值，灵活且零依赖。

# 11 测试工具箱

## 11.1 testing.Short()：长短测试分类

使用 `go test -short` 跳过耗时测试，实现快速反馈循环：
...
## 11.2 Golden File：快照测试

将函数输出与预先保存的"黄金文件"对比，适合格式化输出、序列化结果等：

```bash
# 对比现有快照
go test -run TestGoldenFile ./...

# 重新生成快照
go test -run TestGoldenFile ./... -update
```

> 代码示例：`golden_test.go`、`testdata/user_report.golden`

## 11.3 Example_xxx：文档测试

Example 函数具有双重用途：作为 `go test` 运行的测试 + `godoc` 展示的文档。
...
> 代码示例：`example_test.go`

## 11.4 t.Cleanup()：资源清理

在测试结束时执行清理，支持 LIFO 顺序，可在 helper 函数中注册：

```go
func createTempFile(t *testing.T, dir, content string) string {
    t.Helper()
    // 创建文件...
    t.Cleanup(func() { os.Remove(path) }) // 测试结束后清理
    return path
}
```

与 `defer` 的关键区别：`defer` 在当前函数返回时执行，`t.Cleanup` 在测试结束时执行（可穿越函数边界）。

> 代码示例：`cleanup_test.go`

## 11.5 t.Helper()：自定义断言

标记函数为测试辅助函数，使错误行号指向调用方而非辅助函数内部：
...
本目录的 `trap/` 子目录收集了常见的测试陷阱，每个都是独立的 `package main`，可直接运行演示问题。

## 12.1 循环变量捕获（trap/loop-capture/）

Go 1.21 及之前，`for range` 循环变量在所有迭代中共享地址。配合 goroutine 或 `t.Parallel()` 时，闭包捕获的是最后一个迭代值。

```bash
go run ./goengineering/unit-test/trap/loop-capture/
```

**解决方案**：`tt := tt` 显式捕获，或升级到 Go 1.22+。

## 12.2 Goroutine 泄露（trap/goroutine-leak/）

测试中启动 goroutine 但未等待完成，导致：
- 测试"通过"但 goroutine 还在后台运行
- goroutine 的 panic 不会被测试捕获
- 累积的 goroutine 消耗内存

```bash
go run ./goengineering/unit-test/trap/goroutine-leak/
```

**检测方法**：比较测试前后 `runtime.NumGoroutine()`，或使用 `go.uber.org/goleak`。

## 12.3 共享状态污染（trap/test-pollution/）

包级可变状态导致测试结果依赖执行顺序：

```bash
go run ./goengineering/unit-test/trap/test-pollution/
```

**解决方案**：
1. 避免包级可变状态，通过参数/构造函数传入依赖
2. 每个测试重置状态：`t.Cleanup(func() { ... })`
3. 使用 `t.Setenv()` 代替 `os.Setenv()`（自动恢复）
4. 用 `go test -shuffle=on` 发现隐藏的顺序依赖
...
`performance/` 目录对比了接口调用 vs 直接函数调用的开销：

```bash
go test -bench=. -benchmem ./goengineering/unit-test/performance/
```

结论：接口间接调用的开销在纳秒级（~1ns），对实际业务代码可忽略不计。**不要为了避免接口开销而牺牲可测试性。**

## 13.3 交叉引用

本仓库中其他包含基准测试的模块：

- `lock/performance/`：互斥锁 vs 读写锁 vs atomic 的性能对比
- `sync/`：sync.Pool、sync.Map 等同步原语的基准测试
- `map/`：map 操作的性能特征
- `slice/`：slice 操作的内存分配与性能

这些基准测试均使用相同的方法论：多场景子目录 + `benchstat` 统计分析。